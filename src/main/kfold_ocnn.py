...
for fold, (train_idx, val_idx) in enumerate(kf.split(X_train)):
    ...
    if Config.USE_WANDB:
        wandb.init(project="dl-anomaly-ocnn-10fold", entity="hasan-evci", name="Latent_OCNN_Fold"+str(fold+1), config={
    ...
    # Model
    model = OneClassNN(input_dim, Config.HIDDEN_LAYER_SIZE)
    print(model)
    ...
    for epoch in range(Config.EPOCHS):
        ...
        for data in train_loader:  # Assuming train_loader yields batches of data
            ...
            loss = custom_ocnn_loss(model, r, Config.PARAMETER_NU)(outputs)
            loss.backward()
            optimizer.step()

            # Update r based on quantile of outputs
            with torch.no_grad():
                r = torch.quantile(outputs, Config.PARAMETER_NU).item()
        ...
        if normal_percentage - best_normal_percentage > Config.MIN_DELTA:
            ...
            counter = 0
            # Save the best model
            torch.save(model.state_dict(), 'latent_ocnn_fold{}.pth'.format(fold+1))
        else:
            counter += 1
...
with open('latent_ocnn_fold_performances.pkl', 'wb') as file:
